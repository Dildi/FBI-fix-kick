...
void MyMACAddr::AssingRndMAC()
{
	...
	if (cin.fail() || (selection < 1) || (selection > range))
	{
		...
		cerr << "[Apple]Invalid Selection Input!" << endl;
		return;
	}

	cout << "----------------------------------------------" << endl;
	cout << "[Apple]Selected Adapter is: " << list.at(selection - 1) << endl;
	cout << "[Apple]Old MAC: " << AdapterDetails.at(list.at(selection - 1)) << endl;
			...
			if (RegOpenKeyEx(hKey, Name, 0, KEY_ALL_ACCESS, &hSubKey) == ERROR_SUCCESS)
			{
				...
				if (RegQueryValueEx(hSubKey, _T("DriverDesc"), NULL, NULL, Data, &cbData) == ERROR_SUCCESS)
				{
					...
					if (_tcscmp((TCHAR*)Data, wAdapterName) == 0)
					{
						...
						{
							cout << "[Apple] New Random MAC: " << newMAC << endl;
							Sleep(1000);
							Sleep(500);
	...
	{
		cerr << "[Apple]Cannot Access Registry - Open program as admin." << endl;
		return;
	}
	cout << "----------------------------------------------" << endl;
